...
- Current-window tab metadata needed to render the switcher, including each tab's title, URL or pending URL, favicon, position, active state, and recent access time.
...
- To update a locally rendered 3D tab map when the selected tab changes.
...
- `tabs`: used to read the tabs in the current browser window so TabScroll can show the list of tabs, including title, URL, favicon, order, active state, and recent access time; suggest a tab to jump back to; and activate the selected tab.
