...
4. `04-local-by-design.png` — permission clarity and trust
5. `05-light-and-night.png` — theme choice

`template.html` is the editable source. Open it with `?slide=1` through `?slide=5` at a 1280×800 viewport to reproduce each export. Current product captures used by the template are stored in `source/`; `product-demo.html` plus `demo-runtime.js` reproduce the core and onboarding captures using the real extension UI.
